...
static DEFINE_PER_CPU(struct cpufreq_suspend_t, cpufreq_suspend_data);
				...
				unsigned int relation)
{
	...
	mutex_lock(&per_cpu(cpufreq_suspend_data, policy->cpu).suspend_mutex);

	if (target_freq == policy->cur)
		goto done;

	if (per_cpu(cpufreq_suspend_data, policy->cpu).device_suspended) {
	...
	mutex_unlock(&per_cpu(cpufreq_suspend_data, policy->cpu).suspend_mutex);
	return ret;
}
...
static int msm_cpufreq_suspend(void)
{
	...
	for_each_possible_cpu(cpu) {
		mutex_lock(&per_cpu(cpufreq_suspend_data, cpu).suspend_mutex);
		per_cpu(cpufreq_suspend_data, cpu).device_suspended = 1;
		mutex_unlock(&per_cpu(cpufreq_suspend_data, cpu).suspend_mutex);
	}
...
static int msm_cpufreq_resume(void)
{
	...
	for_each_possible_cpu(cpu) {
		per_cpu(cpufreq_suspend_data, cpu).device_suspended = 0;
	}
...
static int __init msm_cpufreq_register(void)
{
	...
	for_each_possible_cpu(cpu) {
		mutex_init(&(per_cpu(cpufreq_suspend_data, cpu).suspend_mutex));
		per_cpu(cpufreq_suspend_data, cpu).device_suspended = 0;
	}
	...
	if (rc < 0) {
		...
		for_each_possible_cpu(cpu)
			mutex_destroy(&(per_cpu(cpufreq_suspend_data, cpu).
					suspend_mutex));
		return rc;
	}
